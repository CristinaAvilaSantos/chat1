...
def main(server_address, info):
    ...
    with Client(address = (server_address, 6000), authkey = b'secret password server') as conn:
        ...
        connected = True
        while connected:
            value = input("Send message ('quit connection')?")
...
if _name_ == '_main_':
    ...
    client_port = 6001
    if len(sys.argv) > 1:
        clien_port = int(sys.arg[1])
    if len(sys.argv) > 2:    
        client_address = sys.arg[2]
    if len(sys.argv) > 3:    
        server_address = sys.arg[3]
    info = {'address' : client_address, 'port' : client_port, 'authkey' :b'secret client server'}
